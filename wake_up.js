...
function getChinaTimeString() { return formatDateTimeInTimeZone(new Date(), TIME_ZONE); }
function getLocalTimeString() { return formatDateTimeInTimeZone(new Date(), TIME_ZONE); }
...
${historyText}`
  ...
  if (!aiText) {
    ...
    barkText = barkText.replace(/^标题[：:]\s*/gm, "").replace(/^正文[：:]\s*/gm, "");
    ...
    if (!eventContent) {
      ...
      let safeTitle = title || "时衍";
      if (/^\d/.test(safeTitle)) safeTitle = "时衍｜" + safeTitle;
...
async function scheduleNextCheck() {
  try {
    try { await fetch(HEARTBEAT_URL, { method: "POST" }); } catch {}
    await runWakeUp();
  } catch (err) {
